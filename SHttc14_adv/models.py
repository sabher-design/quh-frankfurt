...
from SHttc14_adv.user_settings import *
...
class Subsession(BaseSubsession):
    ...
    def creating_session(self):
        print(f"Starting session for SHttc14_adv: {self.__class__.__name__}")
       ...
       # for p in players:
        ...
        for p in players:
            ...
            p.participant.vars['success14'] = [False for n in indices]
            #p.participant.vars['role'] = p.role()
        ...
        for p in players:
            p.participant.vars['val14_others'] = []
            p.participant.vars['other_types_names'] = []
            for t in type_names:
                if p.role() == t:
                    p.participant.vars['val14'] = Constants.val14[type_names.index(t)]
                else:
                    if Constants.nr_types > 1:
                        p.participant.vars['val14_others'].append(Constants.val14[type_names.index(t)])
                        p.participant.vars['other_types_names'] = [t for t in type_names if p.role() != t]
        print(f"others' vals: {p.participant.vars['val14_others']}")
        print(f"own vals: {p.participant.vars['val14']}")
        # ALLOCATE THE CORRECT prio14 VECTOR TO PLAYER (DEPENDING ON ID) ==================== #
        for p in players:
            p.participant.vars['prio14'] = []
            for i in Constants.prio14:
                p.participant.vars['prio14'].extend([(i.index(j) + 1) for j in i if j == p.id_in_group])
        print(f"prio14 in models: {p.participant.vars['prio14']}")
    ...
    def vars_for_admin_report(self):
        ...
        table_nr_tds_prio14 = Constants.nr_courses + 1
        player_prefs = [p.participant.vars['player_prefs'] for p in players]
        last_player_per_group = [i[-1] for i in self.get_group_matrix()]
        player_val14 = [p.participant.vars['val14'] for p in players]
        player_prio14 = [p.participant.vars['prio14'] for p in players]
        types = ['Type ' + str(i) for i in range(1, Constants.nr_types + 1)]
        val14 = [i for i in Constants.val14]
        capacities = [i for i in Constants.capacities]
        decisions = zip(players, player_prefs)
        success14 = [p.participant.vars['success14'] for p in players]
        success14_with_id = zip(players, success14)
        val14_all_types = zip(types, val14)
        prio14_all_players = zip(players, player_prio14)

        data_all = zip(players, player_val14, player_prefs, success14)

        return {
            'indices': indices,
            'players': players,
            'table_nr_tds_decisions': table_nr_tds_decisions,
            'table_nr_tds_prio14': table_nr_tds_prio14,
            'player_prefs': player_prefs,
            'last_player_per_group': last_player_per_group,
            'player_prio14': player_prio14,
            'capacities': capacities,
            'decisions': decisions,
            'success14': success14,
            'success14_with_id': success14_with_id,
            'val14_all_types': val14_all_types,
            'prio14_all_players': prio14_all_players,
...
class Group(BaseGroup):
    ...
    def get_allocation(self):
        ...
        prio14_left = self.deep_copy_priorities(Constants.prio14)
        print(f"priorities left first time: {prio14_left}")
        #prio14_left = Constants.prio14.copy()
        #print(f"priorities left first time: {prio14_left}")

        #(Constants.prio14.copy())
        ...
        while size_counter <= len(players_in_round) +10:
            ...
            # DETERMINE THE TOP prio14 IN THIS ROUND ======================================== #
            top_prio14_in_round = []
            print(f"prio14 left: {prio14_left}")
            for i, j in zip(indices, prio14_left):
                if seats_left[i - 1] > 0:
                    top_prio14_in_round.append([i, j[0]])
                    print(f"top_prio14_in_round: {top_prio14_in_round}")
            # MAKE AN EMPTY LIST THAT CATCHES ALL FOUND CYCLES AND APPEND ALL CYCLES ============ #
            cycles_check = []

            for i in top_prefs_in_round:
                j = next(j for j in top_prio14_in_round if j[0] == i[1])
            ...
            if size_counter > 1:
                ...
                while m < size_counter:
                    for i in cycles_check:
                        ...
                        k = next(k for k in top_prio14_in_round if k[0] == j[-1])
    ...
    def set_payoffs(self):
        ...
        for p in players:
            ...
            p.payoff = p.participant.vars['val14'][matched_course - 1]
            p.participant.vars['success14'][matched_course - 1] = True
            print(
                f"  Matched with Course {matched_course}. Payoff: {p.payoff}. success14: {p.participant.vars['success14'][matched_course - 1]}")
