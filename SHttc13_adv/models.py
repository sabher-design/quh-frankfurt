...
from SHttc13_adv.user_settings import *
...
class Subsession(BaseSubsession):
    ...
    def creating_session(self):
        print(f"Starting session for SHttc13_adv: {self.__class__.__name__}")
       ...
       # for p in players:
        ...
        for p in players:
            ...
            p.participant.vars['success13'] = [False for n in indices]
            #p.participant.vars['role'] = p.role()
        ...
        for p in players:
            p.participant.vars['val13_others'] = []
            p.participant.vars['other_types_names'] = []
            for t in type_names:
                if p.role() == t:
                    p.participant.vars['val13'] = Constants.val13[type_names.index(t)]
                else:
                    if Constants.nr_types > 1:
                        p.participant.vars['val13_others'].append(Constants.val13[type_names.index(t)])
                        p.participant.vars['other_types_names'] = [t for t in type_names if p.role() != t]
        print(f"others' vals: {p.participant.vars['val13_others']}")
        print(f"own vals: {p.participant.vars['val13']}")
        # ALLOCATE THE CORRECT prio13 VECTOR TO PLAYER (DEPENDING ON ID) ==================== #
        for p in players:
            p.participant.vars['prio13'] = []
            for i in Constants.prio13:
                p.participant.vars['prio13'].extend([(i.index(j) + 1) for j in i if j == p.id_in_group])
        print(f"prio13 in models: {p.participant.vars['prio13']}")
    ...
    def vars_for_admin_report(self):
        ...
        table_nr_tds_prio13 = Constants.nr_courses + 1
        player_prefs = [p.participant.vars['player_prefs'] for p in players]
        last_player_per_group = [i[-1] for i in self.get_group_matrix()]
        player_val13 = [p.participant.vars['val13'] for p in players]
        player_prio13 = [p.participant.vars['prio13'] for p in players]
        types = ['Type ' + str(i) for i in range(1, Constants.nr_types + 1)]
        val13 = [i for i in Constants.val13]
        capacities = [i for i in Constants.capacities]
        decisions = zip(players, player_prefs)
        success13 = [p.participant.vars['success13'] for p in players]
        success13_with_id = zip(players, success13)
        val13_all_types = zip(types, val13)
        prio13_all_players = zip(players, player_prio13)

        data_all = zip(players, player_val13, player_prefs, success13)

        return {
            'indices': indices,
            'players': players,
            'table_nr_tds_decisions': table_nr_tds_decisions,
            'table_nr_tds_prio13': table_nr_tds_prio13,
            'player_prefs': player_prefs,
            'last_player_per_group': last_player_per_group,
            'player_prio13': player_prio13,
            'capacities': capacities,
            'decisions': decisions,
            'success13': success13,
            'success13_with_id': success13_with_id,
            'val13_all_types': val13_all_types,
            'prio13_all_players': prio13_all_players,
...
class Group(BaseGroup):
    ...
    def get_allocation(self):
        ...
        prio13_left = self.deep_copy_priorities(Constants.prio13)
        print(f"priorities left first time: {prio13_left}")
        #prio13_left = Constants.prio13.copy()
        #print(f"priorities left first time: {prio13_left}")

        #(Constants.prio13.copy())
        ...
        while size_counter <= len(players_in_round) +10:
            ...
            # DETERMINE THE TOP prio13 IN THIS ROUND ======================================== #
            top_prio13_in_round = []
            print(f"prio13 left: {prio13_left}")
            for i, j in zip(indices, prio13_left):
                if seats_left[i - 1] > 0:
                    top_prio13_in_round.append([i, j[0]])
                    print(f"top_prio13_in_round: {top_prio13_in_round}")
            # MAKE AN EMPTY LIST THAT CATCHES ALL FOUND CYCLES AND APPEND ALL CYCLES ============ #
            cycles_check = []

            for i in top_prefs_in_round:
                j = next(j for j in top_prio13_in_round if j[0] == i[1])
            ...
            if size_counter > 1:
                ...
                while m < size_counter:
                    for i in cycles_check:
                        ...
                        k = next(k for k in top_prio13_in_round if k[0] == j[-1])
    ...
    def set_payoffs(self):
        ...
        for p in players:
            ...
            p.payoff = p.participant.vars['val13'][matched_course - 1]
            p.participant.vars['success13'][matched_course - 1] = True
            print(
                f"  Matched with Course {matched_course}. Payoff: {p.payoff}. success13: {p.participant.vars['success13'][matched_course - 1]}")
