...
from SHttc9_adv.user_settings import *
...
class Subsession(BaseSubsession):
    ...
    def creating_session(self):
        print(f"Starting session for SHttc9_adv: {self.__class__.__name__}")
       ...
       # for p in players:
        ...
        print(f"others' vals: {p.participant.vars['val9_others']}")
        print(f"own vals: {p.participant.vars['val9']}")
        ...
        print(f"prio9 in models: {p.participant.vars['prio9']}")
...
class Group(BaseGroup):
    ...
    def get_allocation(self):
        ...
        prio9_left = self.deep_copy_priorities(Constants.prio9)
        print(f"priorities left first time: {prio9_left}")
        #prio9_left = Constants.prio9.copy()
        #print(f"priorities left first time: {prio9_left}")

        #(Constants.prio9.copy())
        ...
        while size_counter <= len(players_in_round) +10:
            ...
            # DETERMINE THE TOP prio9 IN THIS ROUND ======================================== #
            top_prio9_in_round = []
            print(f"prio9 left: {prio9_left}")
            for i, j in zip(indices, prio9_left):
                if seats_left[i - 1] > 0:
                    top_prio9_in_round.append([i, j[0]])
                    print(f"top_prio9_in_round: {top_prio9_in_round}")
            # MAKE AN EMPTY LIST THAT CATCHES ALL FOUND CYCLES AND APPEND ALL CYCLES ============ #
            cycles_check = []

            for i in top_prefs_in_round:
                j = next(j for j in top_prio9_in_round if j[0] == i[1])
            ...
            if size_counter > 1:
                ...
                while m < size_counter:
                    for i in cycles_check:
                        ...
                        k = next(k for k in top_prio9_in_round if k[0] == j[-1])
    ...
    def set_payoffs(self):
        ...
        for p in players:
            ...
            p.payoff = p.participant.vars['val9'][matched_course - 1]
            p.participant.vars['success9'][matched_course - 1] = True
            print(
                f"  Matched with Course {matched_course}. Payoff: {p.payoff}. success9: {p.participant.vars['success9'][matched_course - 1]}")
