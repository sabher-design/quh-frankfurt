...
from SHttc7_adv.user_settings import *
...
class Subsession(BaseSubsession):
    ...
    def creating_session(self):
        print(f"Starting session for SHttc7_adv: {self.__class__.__name__}")
       ...
       # for p in players:
        ...
        for p in players:
            ...
            p.participant.vars['success7'] = [False for n in indices]
            #p.participant.vars['role'] = p.role()
        ...
        for p in players:
            p.participant.vars['val7_others'] = []
            p.participant.vars['other_types_names'] = []
            for t in type_names:
                if p.role() == t:
                    p.participant.vars['val7'] = Constants.val7[type_names.index(t)]
                else:
                    if Constants.nr_types > 1:
                        p.participant.vars['val7_others'].append(Constants.val7[type_names.index(t)])
                        p.participant.vars['other_types_names'] = [t for t in type_names if p.role() != t]
        print(f"others' vals: {p.participant.vars['val7_others']}")
        print(f"own vals: {p.participant.vars['val7']}")
        # ALLOCATE THE CORRECT prio7 VECTOR TO PLAYER (DEPENDING ON ID) ==================== #
        for p in players:
            p.participant.vars['prio7'] = []
            for i in Constants.prio7:
                p.participant.vars['prio7'].extend([(i.index(j) + 1) for j in i if j == p.id_in_group])
        print(f"prio7 in models: {p.participant.vars['prio7']}")
    ...
    def vars_for_admin_report(self):
        ...
        table_nr_tds_prio7 = Constants.nr_courses + 1
        player_prefs = [p.participant.vars['player_prefs'] for p in players]
        last_player_per_group = [i[-1] for i in self.get_group_matrix()]
        player_val7 = [p.participant.vars['val7'] for p in players]
        player_prio7 = [p.participant.vars['prio7'] for p in players]
        types = ['Type ' + str(i) for i in range(1, Constants.nr_types + 1)]
        val7 = [i for i in Constants.val7]
        capacities = [i for i in Constants.capacities]
        decisions = zip(players, player_prefs)
        success7 = [p.participant.vars['success7'] for p in players]
        success7_with_id = zip(players, success7)
        val7_all_types = zip(types, val7)
        prio7_all_players = zip(players, player_prio7)

        data_all = zip(players, player_val7, player_prefs, success7)

        return {
            'indices': indices,
            'players': players,
            'table_nr_tds_decisions': table_nr_tds_decisions,
            'table_nr_tds_prio7': table_nr_tds_prio7,
            'player_prefs': player_prefs,
            'last_player_per_group': last_player_per_group,
            'player_prio7': player_prio7,
            'capacities': capacities,
            'decisions': decisions,
            'success7': success7,
            'success7_with_id': success7_with_id,
            'val7_all_types': val7_all_types,
            'prio7_all_players': prio7_all_players,
...
class Group(BaseGroup):
    ...
    def get_allocation(self):
        ...
        prio7_left = self.deep_copy_priorities(Constants.prio7)
        print(f"priorities left first time: {prio7_left}")
        #prio7_left = Constants.prio7.copy()
        #print(f"priorities left first time: {prio7_left}")

        #(Constants.prio7.copy())
        ...
        while size_counter <= len(players_in_round) +10:
            ...
            # DETERMINE THE TOP prio7 IN THIS ROUND ======================================== #
            top_prio7_in_round = []
            print(f"prio7 left: {prio7_left}")
            for i, j in zip(indices, prio7_left):
                if seats_left[i - 1] > 0:
                    top_prio7_in_round.append([i, j[0]])
                    print(f"top_prio7_in_round: {top_prio7_in_round}")
            # MAKE AN EMPTY LIST THAT CATCHES ALL FOUND CYCLES AND APPEND ALL CYCLES ============ #
            cycles_check = []

            for i in top_prefs_in_round:
                j = next(j for j in top_prio7_in_round if j[0] == i[1])
            ...
            if size_counter > 1:
                ...
                while m < size_counter:
                    for i in cycles_check:
                        ...
                        k = next(k for k in top_prio7_in_round if k[0] == j[-1])
    ...
    def set_payoffs(self):
        ...
        for p in players:
            ...
            p.payoff = p.participant.vars['val7'][matched_course - 1]
            p.participant.vars['success7'][matched_course - 1] = True
            print(
                f"  Matched with Course {matched_course}. Payoff: {p.payoff}. success7: {p.participant.vars['success7'][matched_course - 1]}")
