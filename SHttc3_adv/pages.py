...
# =========================================================================================== #
print(f"val3 SHttc3_adv: {Constants.val3}")
print(f"prio3 SHttc3_adv: {Constants.prio3}")
...
class Results(Page):
    ...
    def before_next_page(self):
        ...
        self.participant.vars['SHttc3_adv_payoff'] = "{:.0f}".format(base_payoff)
        self.participant.vars['SHttc3_adv_payoff_half'] = "{:.2f}".format(half_payoff)
        self.participant.vars['SHttc3_adv_payoff_final'] = "{:.2f}".format(final_payoff)

    def vars_for_template(self):
        player_prefs = [i[0] for i in self.participant.vars['player_prefs']]
        success3 = [i for i in self.participant.vars['success3']]
        #base_payoff = self.participant.vars.get('SHttc3_adv_payoff', 0)
