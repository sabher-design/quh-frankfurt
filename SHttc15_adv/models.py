...
from SHttc15_adv.user_settings import *
...
class Subsession(BaseSubsession):
    ...
    def creating_session(self):
        print(f"Starting session for SHttc15_adv: {self.__class__.__name__}")
       ...
       # for p in players:
        ...
        for p in players:
            ...
            p.participant.vars['success15'] = [False for n in indices]
            #p.participant.vars['role'] = p.role()
        ...
        for p in players:
            p.participant.vars['val15_others'] = []
            p.participant.vars['other_types_names'] = []
            for t in type_names:
                if p.role() == t:
                    p.participant.vars['val15'] = Constants.val15[type_names.index(t)]
                else:
                    if Constants.nr_types > 1:
                        p.participant.vars['val15_others'].append(Constants.val15[type_names.index(t)])
                        p.participant.vars['other_types_names'] = [t for t in type_names if p.role() != t]
        print(f"others' vals: {p.participant.vars['val15_others']}")
        print(f"own vals: {p.participant.vars['val15']}")
        # ALLOCATE THE CORRECT prio15 VECTOR TO PLAYER (DEPENDING ON ID) ==================== #
        for p in players:
            p.participant.vars['prio15'] = []
            for i in Constants.prio15:
                p.participant.vars['prio15'].extend([(i.index(j) + 1) for j in i if j == p.id_in_group])
        print(f"prio15 in models: {p.participant.vars['prio15']}")
    ...
    def vars_for_admin_report(self):
        ...
        table_nr_tds_prio15 = Constants.nr_courses + 1
        player_prefs = [p.participant.vars['player_prefs'] for p in players]
        last_player_per_group = [i[-1] for i in self.get_group_matrix()]
        player_val15 = [p.participant.vars['val15'] for p in players]
        player_prio15 = [p.participant.vars['prio15'] for p in players]
        types = ['Type ' + str(i) for i in range(1, Constants.nr_types + 1)]
        val15 = [i for i in Constants.val15]
        capacities = [i for i in Constants.capacities]
        decisions = zip(players, player_prefs)
        success15 = [p.participant.vars['success15'] for p in players]
        success15_with_id = zip(players, success15)
        val15_all_types = zip(types, val15)
        prio15_all_players = zip(players, player_prio15)

        data_all = zip(players, player_val15, player_prefs, success15)

        return {
            'indices': indices,
            'players': players,
            'table_nr_tds_decisions': table_nr_tds_decisions,
            'table_nr_tds_prio15': table_nr_tds_prio15,
            'player_prefs': player_prefs,
            'last_player_per_group': last_player_per_group,
            'player_prio15': player_prio15,
            'capacities': capacities,
            'decisions': decisions,
            'success15': success15,
            'success15_with_id': success15_with_id,
            'val15_all_types': val15_all_types,
            'prio15_all_players': prio15_all_players,
...
class Group(BaseGroup):
    ...
    def get_allocation(self):
        ...
        print(f"priorities left first time: {prio15_left}")
        #prio15_left = Constants.prio15.copy()
        #print(f"priorities left first time: {prio15_left}")
        ...
        while size_counter <= len(players_in_round) +10:
            ...
            # DETERMINE THE TOP prio15 IN THIS ROUND ======================================== #
            top_prio15_in_round = []
            print(f"prio15 left: {prio15_left}")
            for i, j in zip(indices, prio15_left):
                if seats_left[i - 1] > 0:
                    top_prio15_in_round.append([i, j[0]])
                    print(f"top_prio15_in_round: {top_prio15_in_round}")
    ...
    def set_payoffs(self):
        ...
        for p in players:
            ...
            p.payoff = p.participant.vars['val15'][matched_course - 1]
            p.participant.vars['success15'][matched_course - 1] = True
            print(
                f"  Matched with Course {matched_course}. Payoff: {p.payoff}. success15: {p.participant.vars['success15'][matched_course - 1]}")
