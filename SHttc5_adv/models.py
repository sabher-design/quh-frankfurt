...
from SHttc5_adv.user_settings import *
...
class Subsession(BaseSubsession):
    ...
    def creating_session(self):
        print(f"Starting session for SHttc5_adv: {self.__class__.__name__}")
       ...
       # for p in players:
        ...
        for p in players:
            ...
            p.participant.vars['success5'] = [False for n in indices]
            #p.participant.vars['role'] = p.role()
        ...
        for p in players:
            p.participant.vars['val5_others'] = []
            p.participant.vars['other_types_names'] = []
            for t in type_names:
                if p.role() == t:
                    p.participant.vars['val5'] = Constants.val5[type_names.index(t)]
                else:
                    if Constants.nr_types > 1:
                        p.participant.vars['val5_others'].append(Constants.val5[type_names.index(t)])
                        p.participant.vars['other_types_names'] = [t for t in type_names if p.role() != t]
        print(f"others' vals: {p.participant.vars['val5_others']}")
        print(f"own vals: {p.participant.vars['val5']}")
        # ALLOCATE THE CORRECT prio5 VECTOR TO PLAYER (DEPENDING ON ID) ==================== #
        for p in players:
            p.participant.vars['prio5'] = []
            for i in Constants.prio5:
                p.participant.vars['prio5'].extend([(i.index(j) + 1) for j in i if j == p.id_in_group])
        print(f"prio5 in models: {p.participant.vars['prio5']}")
    ...
    def vars_for_admin_report(self):
        ...
        table_nr_tds_prio5 = Constants.nr_courses + 1
        player_prefs = [p.participant.vars['player_prefs'] for p in players]
        last_player_per_group = [i[-1] for i in self.get_group_matrix()]
        player_val5 = [p.participant.vars['val5'] for p in players]
        player_prio5 = [p.participant.vars['prio5'] for p in players]
        types = ['Type ' + str(i) for i in range(1, Constants.nr_types + 1)]
        val5 = [i for i in Constants.val5]
        capacities = [i for i in Constants.capacities]
        decisions = zip(players, player_prefs)
        success5 = [p.participant.vars['success5'] for p in players]
        success5_with_id = zip(players, success5)
        val5_all_types = zip(types, val5)
        prio5_all_players = zip(players, player_prio5)

        data_all = zip(players, player_val5, player_prefs, success5)

        return {
            'indices': indices,
            'players': players,
            'table_nr_tds_decisions': table_nr_tds_decisions,
            'table_nr_tds_prio5': table_nr_tds_prio5,
            'player_prefs': player_prefs,
            'last_player_per_group': last_player_per_group,
            'player_prio5': player_prio5,
            'capacities': capacities,
            'decisions': decisions,
            'success5': success5,
            'success5_with_id': success5_with_id,
            'val5_all_types': val5_all_types,
            'prio5_all_players': prio5_all_players,
...
class Group(BaseGroup):
    ...
    def get_allocation(self):
        ...
        prio5_left = self.deep_copy_priorities(Constants.prio5)
        print(f"priorities left first time: {prio5_left}")
        #prio5_left = Constants.prio5.copy()
        #print(f"priorities left first time: {prio5_left}")

        #(Constants.prio5.copy())
        ...
        while size_counter <= len(players_in_round) +10:
            ...
            # DETERMINE THE TOP prio5 IN THIS ROUND ======================================== #
            top_prio5_in_round = []
            print(f"prio5 left: {prio5_left}")
            for i, j in zip(indices, prio5_left):
                if seats_left[i - 1] > 0:
                    top_prio5_in_round.append([i, j[0]])
                    print(f"top_prio5_in_round: {top_prio5_in_round}")
            # MAKE AN EMPTY LIST THAT CATCHES ALL FOUND CYCLES AND APPEND ALL CYCLES ============ #
            cycles_check = []

            for i in top_prefs_in_round:
                j = next(j for j in top_prio5_in_round if j[0] == i[1])
            ...
            if size_counter > 1:
                ...
                while m < size_counter:
                    for i in cycles_check:
                        ...
                        k = next(k for k in top_prio5_in_round if k[0] == j[-1])
    ...
    def set_payoffs(self):
        ...
        for p in players:
            ...
            p.payoff = p.participant.vars['val5'][matched_course - 1]
            p.participant.vars['success5'][matched_course - 1] = True
            print(
                f"  Matched with Course {matched_course}. Payoff: {p.payoff}. success5: {p.participant.vars['success5'][matched_course - 1]}")
