...
from SHttc20_adv.user_settings import *
...
class Subsession(BaseSubsession):
    ...
    def creating_session(self):
        print(f"Starting session for SHttc20_adv: {self.__class__.__name__}")
       ...
       # for p in players:
        ...
        print(f"others' vals: {p.participant.vars['val20_others']}")
        print(f"own vals: {p.participant.vars['val20']}")
        ...
        print(f"prio20 in models: {p.participant.vars['prio20']}")
...
class Group(BaseGroup):
    ...
    def get_allocation(self):
        ...
        prio20_left = self.deep_copy_priorities(Constants.prio20)
        print(f"priorities left first time: {prio20_left}")
        #prio20_left = Constants.prio20.copy()
        #print(f"priorities left first time: {prio20_left}")

        #(Constants.prio20.copy())
        ...
        while size_counter <= len(players_in_round) +10:
            ...
            # DETERMINE THE TOP prio20 IN THIS ROUND ======================================== #
            top_prio20_in_round = []
            print(f"prio20 left: {prio20_left}")
            for i, j in zip(indices, prio20_left):
                if seats_left[i - 1] > 0:
                    top_prio20_in_round.append([i, j[0]])
                    print(f"top_prio20_in_round: {top_prio20_in_round}")
            # MAKE AN EMPTY LIST THAT CATCHES ALL FOUND CYCLES AND APPEND ALL CYCLES ============ #
            cycles_check = []

            for i in top_prefs_in_round:
                j = next(j for j in top_prio20_in_round if j[0] == i[1])
            ...
            if size_counter > 1:
                ...
                while m < size_counter:
                    for i in cycles_check:
                        ...
                        k = next(k for k in top_prio20_in_round if k[0] == j[-1])
    ...
    def set_payoffs(self):
        ...
        for p in players:
            ...
            p.payoff = p.participant.vars['val20'][matched_course - 1]
            p.participant.vars['success20'][matched_course - 1] = True
            print(
                f"  Matched with Course {matched_course}. Payoff: {p.payoff}. success20: {p.participant.vars['success20'][matched_course - 1]}")
